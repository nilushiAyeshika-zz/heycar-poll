...
export const IconTitleStyles = styled.div<IconTitleProps>`
  ...
  align-items: center;
  & > div:nth-child(1) {
    background: ${(props) => (props.backgroundColor ? props.backgroundColor : appTheme.colors.DEFAULT_ICON_BG)};
  }
  ${(props) => (props.margin && `margin: ${props.margin}`)};
  ${(props) => (props.padding && `padding: ${props.padding}`)};
  svg {
    color: ${(props) => (props.isLightIcon ? appTheme.colors.TYPHOGRAPHY_DARK : appTheme.colors.TYPHOGRAPHY_WHITE)};
