...
const QuestionScreen: React.FC = (props: any) => {
			...
			{({ values }: any) => (
				<Form>
					<Grid direction="column">
						<label htmlFor="question" className="formik-label">Question:</label>
									...
									values.choices.map((choice: string, index: number) => (
										<Grid key={index} direction="column">
											<Grid direction="column" margin="2rem 0 0 0">
												<label htmlFor={`choices.${index}`} className="formik-label">Choice {index + 1}:</label>
												...
												<ErrorMessage name={`choices.${index}`} component="div" className="formik-error-label" />
											</Grid>
										</Grid>
									))}
								<Button
									...
									hoverColor="button-secondary-hover"
								>
									<Text size="m" color="typo-white">Add Choice</Text>
								</Button>
								<Text className="info-message" size="xs" margin="1rem 0 2rem 0">You can add only maximum 04 choices</Text>
							</Grid>
						)}
					</FieldArray>
					<Grid justifyContent="flex-end">
						...
						>
							<Text size="m" color="typo-white">Add</Text>
						</Button>
					</Grid>
				</Form>
			)}
  ...
  return (
    <QuestionScreenWrapper>
			<Grid alignItems="center" margin="4rem 0" padding="0 5rem" direction="column" className="question-screen-wrapper">
				<Grid>
					<IconTitle
						margin="0 0 4rem 0"
					...
					<Button
						...
						startIcon={<FontAwesomeIcon icon={faPlus} />}
					>
						<Text size="m" color="typo-white">Add New</Text>
					</Button>
				</Grid>
				<QuestionList
					data={questionList}
			...
			{isAddNewOpen && (
				...
				>
					<AddNewWrapper>
						<Text size="l" margin="0 0 3rem 0">Add New Question</Text>
						<Button
